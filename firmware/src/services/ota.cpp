...
static String fetchRemoteFirmwareVersion() {
    ...
    if (code != HTTP_CODE_OK) {
        error("OTA firmware version fetch failed");
        http.end();
        return "";
    }
...
void performFirmwareUpdateOTA(String rmtVersion) {
    ...
    int code = http.GET();
    if (code != HTTP_CODE_OK) {
        error("OTA firmware download failed");
        http.end();
    }
    ...
    if (contentLength <= 0) {
        error("OTA invalid content length");
        http.end();
    }
    else if (!Update.begin(contentLength)) {
        error("OTA firmware update start failed");
        http.end();
    }
    ...
    if (written != contentLength) {
        error("OTA firmware incomplete write");
        Update.abort();
        http.end();
    }
    else if (!Update.end()) {
        error("OTA firmware update end failed");
        http.end();
    }
    else if (!Update.isFinished()) {
        error("OTA firmware update not finished");
        http.end();
    }

    // --- notify firmware update sucess via telegram ---
    sendMsgToTelegram("Firmware updated sucessfully from v" + FW_VERSION + " to v" + rmtVersion);
...
void checkForFirmwareUpdate() {
    ...
    if (remoteVersion.length() == 0) {
        error("No remote firmware version available");
        return;
    }
